...
class Spawns:
    ...
    def spawn_fly_raven(self):
        if self.fly_raven_spawn:
            if self.elapsed_time - self.last_fly_raven_spawn_time > self.fly_raven_spawn:
    ...
    def spawn_ground_raven(self):
        if self.ground_raven_spawn:
            if self.elapsed_time - self.last_ground_raven_spawn_time > self.ground_raven_spawn:
    ...
    def spawn_deadly_raven(self):
        if self.deadly_raven_spawn:
            if self.elapsed_time - self.last_deadly_raven_spawn_time > self.deadly_raven_spawn:
