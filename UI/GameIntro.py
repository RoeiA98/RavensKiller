...
class GameIntro(GameScenes):
    ...
    async def intro_screen_menu(self):
        if self.intro_screen_status:
            self.display_intro()
        elif self.leaderboard_screen_status:
            await self.game_leaderboard.display_leaderboard()
    ...
    def display_intro(self):
        ...
        quit_game_text = self.game_font.render(
            "Quit Game", True, "Black"
        ).convert_alpha()
        quit_game_text_rect = quit_game_text.get_rect(
    ...
    def display_user_name(self):
        name = self.user_display_font.render(
            f"Player: {self.name_input}", True, "White"
        ).convert_alpha()
        self.game_screen.blit(name, (10, 515))

    async def handle_game_intro_events(self):
        for event in pygame.event.get():
            if self.leaderboard_screen_status:
                ...
                if (
                    ...
                    and event.button == 1
                    and self.refresh_leaderboard_rect.collidepoint(event.pos)
                ):
                    print("refresh")
                    self.game_leaderboard.leaderboard_data_fetched = (
                        False  # Reset the flag to fetch data again
                    )
                    await self.game_leaderboard.fetch_leaderboard_data()

                # press back button
                if (
                    event.type == pygame.MOUSEBUTTONDOWN
                    and event.button == 1
                    and self.back_leaderboard_rect.collidepoint(event.pos)
                ):  # Quit Game
                    print("back intro")
                    self.leaderboard_screen_status = False
                    self.intro_screen_status = True
                    self.game_leaderboard.running = False
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    elif len(self.name_input) < 10 and (
                ...
                if (
                    event.type == pygame.MOUSEBUTTONDOWN
                    and event.button == 1
                    and self.leaderboard_button_rect.collidepoint(event.pos)
                ):
                    self.intro_screen_status = False
                    self.leaderboard_screen_status = True
                    self.game_leaderboard.running = True

                # press quit game
                if (
                    event.type == pygame.MOUSEBUTTONDOWN
                    and event.button == 1
                    and self.quit_button_rect.collidepoint(event.pos)
                ):  # Quit Game
                    pygame.quit()
                    exit()
